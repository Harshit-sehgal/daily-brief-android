...
object CriticalPathEngine {
  fun analyze(
    ...
    blocks
      ...
      .forEach { block ->
        when {
          ...
          runCatching { GuardedArithmetic.subtractExact(block.endAt, block.startAt) }.isFailure -> {
            blockedItems += block.planItemId
            issues +=
              CriticalPathIssue(
          ...
          val totalMillis =
            taskBlocks.fold(0L as Long?) { total, block ->
              if (total == null) null
                ...
                val blockMillis =
                  runCatching { GuardedArithmetic.subtractExact(block.endAt, block.startAt) }.getOrNull()
                    ?: return@fold null
                runCatching { GuardedArithmetic.addExact(total, blockMillis) }.getOrNull()
              }
            }
          if (totalMillis == null) {
            durationById[itemId] = null
    ...
    topology.order.forEach { itemId ->
      outgoing[itemId].orEmpty().forEach { edge ->
        val candidate =
          runCatching {
              GuardedArithmetic.addExact(earliest.getValue(itemId), edge.startOffsetMinutes)
            }
            .getOrNull()
        if (candidate == null) {
          issues +=
    ...
    byId.keys.sorted().forEach { itemId ->
      val finish =
        runCatching {
            GuardedArithmetic.addExact(earliest.getValue(itemId), durationById.getValue(itemId)!!)
          }
          .getOrNull()
      if (finish == null) {
        issues +=
    ...
    byId.keys.sorted().forEach { itemId ->
      val value =
        runCatching { GuardedArithmetic.subtractExact(projectDuration, durationById.getValue(itemId)!!) }
          .getOrNull()
      if (value == null) {
        issues +=
          CriticalPathIssue(
    ...
    topology.order.asReversed().forEach { itemId ->
      outgoing[itemId].orEmpty().forEach { edge ->
        val candidate =
          runCatching {
              GuardedArithmetic.subtractExact(
    ...
    val timings =
      byId.keys.sorted().map { itemId ->
        ...
        val lateFinish = runCatching { GuardedArithmetic.addExact(lateStart, duration) }.getOrNull()
        val slack = runCatching { GuardedArithmetic.subtractExact(lateStart, earlyStart) }.getOrNull()
  ...
  private fun generalizedOffset(
    ...
    runCatching {
        when (type) {
          PlanDependencyType.FINISH_TO_START -> GuardedArithmetic.addExact(predecessorDuration, lag)
          PlanDependencyType.START_TO_START -> lag
          PlanDependencyType.FINISH_TO_FINISH ->
            GuardedArithmetic.addExact(GuardedArithmetic.subtractExact(predecessorDuration, successorDuration), lag)
          PlanDependencyType.START_TO_FINISH -> GuardedArithmetic.subtractExact(lag, successorDuration)
...
/**
 * Deterministic lexicographic min-heap of strings, standing in for `java.util.PriorityQueue`.
 *
 * The ordering that matters is the removal order: `PriorityQueue` pops the smallest remaining
 * string, and the tie-break that order gives `topologicalOrder` is what the critical-path tests
 * pin. A binary heap with natural string order reproduces it exactly.
 */
private class MinStringHeap {
  ...
  fun remove(): String {
    val root = elements[0]
    val last = elements.removeAt(elements.lastIndex)
    if (elements.isNotEmpty()) {
      ...
      while (true) {
        ...
        val right = left + 1
        var smallest = index
        if (left < elements.size && elements[left] < elements[smallest]) smallest = left
        if (right < elements.size && elements[right] < elements[smallest]) smallest = right
        if (smallest == index) break
        val swap = elements[index]
